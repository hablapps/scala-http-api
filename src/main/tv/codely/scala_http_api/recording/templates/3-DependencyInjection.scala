...
/**
 * 3. Hacen falta adaptadores para las instancias de MessagePublisher. 
...
object DerivedInstancesI{
  ...
  implicit def fromIdToFutureMp(mp: MessagePublisher[Id])(implicit
...
object DerivedInstancesII{
  ...
  implicit def messagePublisherTransIO(mp: MessagePublisher[Id]): MessagePublisher[IO] = 
  ...
  implicit def messagePublisherTrans[P[_], Q[_]](
    mp: MessagePublisher[P])(implicit
    Q: P ~> Q): MessagePublisher[Q] = 
    new MessagePublisher[Q]{
      def publish(message: Message): Q[Unit] = 
        Q(mp.publish(message))
    }

  implicit def fromIdToFuture(implicit ec: ExecutionContext): Id ~> Future = 
    new (Id ~> Future){
      def apply[T](p: Id[T]): Future[T] =  // Id[T] => Future[T]
        Future(p)
    }
