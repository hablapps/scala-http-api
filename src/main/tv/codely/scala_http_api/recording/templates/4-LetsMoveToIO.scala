...
object BaseInstancesII{
  ...
  import cats.implicits._, cats.Monad

  final case class DoobieMySqlUserRepository[P[_]]()(implicit
    db: DoobieDbConnection[P], 
    M: Monad[P]) extends UserRepository[P] {
...
object DerivedInstancesIII{
  ...
  implicit def fromIdToIO: Id ~> IO = new (Id ~> IO){
